...
void http_receice(String header, String content, int code, unsigned long content_len) {
...
void setup() {
  ...
  Serial.println("\nNBIoT HTTP GET Custom Header Demo!");
  ...
  // Set http recive callback function
  WiSEN.NBIoT.setHTTPHandler(http_receice);
...
void loop() {
  if( millis()-update_t >= INTERVAL_TIME ) {
    ...
    String PATH = "/get?";
    PATH += "id=" + String(id) + "&";
    PATH += "hwid=" + String(hwid) + "&";
    PATH += "imsi=" + String(imsi) + "&";
    PATH += "ip=" + String(ip) + "&";
    PATH += "rssi=" + String(rssi) + "&";
    PATH += "temperature=" + String(temp) + "&";
    PATH += "humidity=" + String(humidity);
    
    String URI = HOST + PATH;
    Serial.print("GET => http://"); Serial.println(URI);
    
    // ***** HTTP ****
    // Http Get Custom header
    String HEADER = "GET " + PATH + " HTTP/1.1\r\n";
    HEADER += "Host: " + HOST + "\r\n"; 
    WiSEN.NBIoT.HTTP(80, URI, HEADER);
    
    // Or Http Get Custom header with header only response
    //String HEADER = "HEAD " + PATH + " HTTP/1.1\r\n";
    //HEADER += "Host: " + HOST + "\r\n"; 
    //WiSEN.NBIoT.HTTP(80, URI, HEADER);
