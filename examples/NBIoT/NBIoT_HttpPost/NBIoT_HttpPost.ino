...
void loop() {
  if( millis()-update_t >= INTERVAL_TIME ) {
    ...
    DATA += "hwid=" + String(hwid) + "&";
    DATA += "imsi=" + String(imsi) + "&";
    DATA += "ip=" + String(ip) + "&";
    ...
    // ***** HTTP ****
    // Http Post
    WiSEN.NBIoT.httpPOST(URI, DATA);
    // Or Http Post with port define
    //WiSEN.NBIoT.httpPOST(80, URI, DATA);
